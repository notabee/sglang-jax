# Adapted from https://github.com/vllm-project/tpu-inference/blob/main/tpu_inference/kernels/megablox/gmm_v2.py
import dataclasses
import functools
...
class IndexMaps:
    ...
    def rhs_scale_index_map(self, n_id: jax.Array, gm_id: jax.Array, _: jax.Array):
        group_id = self.metadata_ref.gm_id_to_group_id[gm_id]
...
def inner_kernel(
    ...
    *,
    ...
    def _matmul(is_first_k_step: bool, is_last_k_step: bool):
        ...
        lax.cond(jnp.any(jnp.isnan(tiled_lhs)), lambda: jax.debug.print("NaN in tiled_lhs"), lambda: None)
        lax.cond(jnp.any(jnp.isnan(tiled_rhs)), lambda: jax.debug.print("NaN in tiled_rhs"), lambda: None)
        ...
        if cfgs.lhs_cfgs.quant_dtype is None:
            ...
            for start_n in range(0, cfgs.tiles.tile_n, mxu_size):
                ...
                for start_k in range(0, cfgs.tiles.tile_k, q_block_size):
                    ...
                    lax.cond(jnp.any(jnp.isnan(block_scale)), lambda: jax.debug.print("NaN in block_scale"), lambda: None)
                    ...
                    lax.cond(jnp.any(jnp.isnan(block_acc)), lambda: jax.debug.print("NaN in block_acc"), lambda: None)
        ...
        if is_last_k_step:
            if cfgs.rhs_cfgs.has_scale:
                ...
                lax.cond(jnp.any(jnp.isnan(acc)), lambda: jax.debug.print("NaN in acc after scale"), lambda: None)
            if cfgs.rhs_cfgs.has_bias:
...
def make_gmm_configs(
    ...
    *,
    ...
    dims = validate_inputs(lhs, rhs, rhs_scale, rhs_bias, group_sizes, group_offset)
    ...
    if acc_dtype is None:
        acc_dtype = jnp.float32.dtype if lhs_cfgs.quant_dtype is None else jnp.bfloat16.dtype
