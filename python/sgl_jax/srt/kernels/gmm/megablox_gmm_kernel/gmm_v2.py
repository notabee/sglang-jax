...
def inner_kernel(
    ...
    *,
    ...
    def _matmul(is_first_k_step: bool, is_last_k_step: bool):
        ...
        jax.debug.print("tiled_lhs has NaN: {}", jnp.any(jnp.isnan(tiled_lhs)).astype(jnp.int32))
        jax.debug.print("tiled_rhs has NaN: {}", jnp.any(jnp.isnan(tiled_rhs)).astype(jnp.int32))
        ...
        if cfgs.lhs_cfgs.quant_dtype is None:
            ...
            for start_n in range(0, cfgs.tiles.tile_n, mxu_size):
                ...
                for start_k in range(0, cfgs.tiles.tile_k, q_block_size):
                    ...
                    jax.debug.print("block_scale has NaN: {}", jnp.any(jnp.isnan(block_scale)).astype(jnp.int32))
                    ...
                    jax.debug.print("block_acc has NaN: {}", jnp.any(jnp.isnan(block_acc)).astype(jnp.int32))
        ...
        if is_last_k_step:
            if cfgs.rhs_cfgs.has_scale:
                ...
                jax.debug.print("acc after scale has NaN: {}", jnp.any(jnp.isnan(acc)).astype(jnp.int32))
            if cfgs.rhs_cfgs.has_bias:
